...
- `Tanaro.Generators/` — a Roslyn incremental source generator (netstandard2.0). Discovers types flagged with `[FunctionUnderTest<T>]` (via `ForAttributeWithMetadataName`, so discovery is driven by syntax in the *consuming* project, not by scanning referenced assemblies) and emits typed `FunctionHost` extension methods for every method on `T` carrying `[Microsoft.Azure.Functions.Worker.Function("Name")]`. Generated methods land in the `Tanaro.Generated` namespace — consumers need `using Tanaro.Generated;`.
