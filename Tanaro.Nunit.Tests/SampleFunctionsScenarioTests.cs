...
public class SampleFunctionsScenarioTests
{
    ...
    public async Task FunctionDefinitionMetadataIsPopulated()
    {
        ...
        Assert.That(result.FunctionContext.FunctionDefinition.Name, Is.EqualTo("SampleWithContext"));
        Assert.That(result.FunctionContext.FunctionDefinition.Id, Is.EqualTo("SampleWithContext"));
        Assert.That(result.FunctionContext.FunctionId, Is.EqualTo(result.FunctionContext.FunctionDefinition.Id));
    }
    ...
    public async Task FunctionDefinitionInputBindingsAreExtractedFromTriggerAttribute()
    {
        ...
        var binding = result.FunctionContext.FunctionDefinition.InputBindings["eventData"];
    ...
    public async Task FunctionDefinitionOutputBindingsAreExtractedFromReturnTypeProperties()
    {
        ...
        var result = await AppUnderTest.Host.Run<SampleFunctions>().SampleWithMultiOutput(s => s.Execute([eventData]));

        var binding = result.FunctionContext.FunctionDefinition.OutputBindings["Message"];
        Assert.That(binding.Type, Is.EqualTo("eventHub"));
        Assert.That(binding.Direction, Is.EqualTo(BindingDirection.Out));
        Assert.That(result.FunctionContext.FunctionDefinition.OutputBindings.ContainsKey("Note"), Is.False);
    }
    ...
    public async Task OutputBindingValueIsCapturedAfterInvocation()
    {
        ...
        var outputBindingData = ((DummyFunctionContext)result.FunctionContext).OutputBindingData;
    ...
    public async Task NullOutputBindingValueIsNotCaptured()
    {
        ...
        var result = await AppUnderTest.Host.Run<SampleFunctions>().SampleWithNullOutput(s => s.Execute([eventData]));

        Assert.That(((DummyFunctionContext)result.FunctionContext).OutputBindingData.ContainsKey("Message"), Is.False);
    }
    ...
    public async Task ReturnBoundFunctionsDoNotPopulateOutputBindingData()
    {
        ...
        var result = await AppUnderTest.Host.Run<SampleFunctions>().SampleReturnBinding(s => s.Execute([eventData]));

        Assert.That(result.Value, Is.EqualTo("x"));
        Assert.That(result.FunctionContext.FunctionDefinition.OutputBindings.ContainsKey("$return"), Is.True);
        Assert.That(((DummyFunctionContext)result.FunctionContext).OutputBindingData, Is.Empty);
    }

    [Test]
    public async Task WarningIsLoggedWhenNoDataReceived()
    {
        var result = await AppUnderTest.Host.Run<SampleFunctions>().SampleWithLogging(s => s.Execute([]));

        var entries = AppUnderTest.Logs.EntriesFor(result.FunctionContext.InvocationId);
    ...
    public async Task InfoIsLoggedWhenDataReceived()
    {
        ...
        var result = await AppUnderTest.Host.Run<SampleFunctions>().SampleWithLogging(s => s.Execute([eventData]));
    ...
    public async Task LogsFromOneInvocationAreNotVisibleOnAnother()
    {
        ...
        var emptyResult = await AppUnderTest.Host.Run<SampleFunctions>().SampleWithLogging(s => s.Execute([]));
        var dataResult = await AppUnderTest.Host.Run<SampleFunctions>().SampleWithLogging(s => s.Execute([eventData]));

        var emptyEntries = AppUnderTest.Logs.EntriesFor(emptyResult.FunctionContext.InvocationId);
        var dataEntries = AppUnderTest.Logs.EntriesFor(dataResult.FunctionContext.InvocationId);
